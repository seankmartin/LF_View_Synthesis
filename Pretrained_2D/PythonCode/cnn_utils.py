...
def load_from_checkpoint(model, optimizer, args, config):
    ...
    if os.path.isfile(resume_location):
        ...
        model.load_state_dict(checkpoint["model"].state_dict(), first=args.first)
...
def load_weights(model, args, config, frozen=False):
    ...
    if os.path.isfile(weights_location):
        ...
        if 'model' in weights:
            model.load_state_dict(weights['model'].state_dict(), frozen, first=args.first)
        else:
            model.load_state_dict(weights, frozen, first=args.first)
    else:
